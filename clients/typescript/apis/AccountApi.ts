...
export class AccountApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async accountBalance(accountBalanceRequest: AccountBalanceRequest, _options?: Configuration): Promise<RequestContext> {
        ...
        if (accountBalanceRequest === null || accountBalanceRequest === undefined) {
            throw new RequiredError("AccountApi", "accountBalance", "accountBalanceRequest");
        }
     ...
     */
    public async accountCoins(accountCoinsRequest: AccountCoinsRequest, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
        ...
        if (accountCoinsRequest === null || accountCoinsRequest === undefined) {
            throw new RequiredError("AccountApi", "accountCoins", "accountCoinsRequest");
        }
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(accountCoinsRequest, "AccountCoinsRequest", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class AccountApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to accountCoins
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async accountCoins(response: ResponseContext): Promise<AccountCoinsResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AccountCoinsResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "AccountCoinsResponse", ""
            ) as AccountCoinsResponse;
            return body;
        }
        if (isCodeInRange("500", response.httpStatusCode)) {
            const body: ErrorResponse = ObjectSerializer.deserialize(
